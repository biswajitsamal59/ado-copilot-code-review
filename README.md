...
Automated pull request code reviews powered by the GitHub Copilot CLI. Get AI-driven feedback on your PRs directly in Azure DevOps.
...
1. Fetches pull request details, file diffs, and existing comments from Azure DevOps
2. Optionally fetches JIRA ticket details extracted from the PR description
3. Invokes the GitHub Copilot CLI to analyze the changes
...
- **GitHub Copilot**: An active GitHub Copilot subscription and a GitHub PAT with Copilot access permissions
...
| `organization` | No | Auto-detected | Azure DevOps organization name for cloud-hosted instances |
| `collectionUri` | No | `$(System.CollectionUri)` | Azure DevOps collection URI (use for on-prem instances) |
...
| `includeWorkItems` | No | `false` | Extract JIRA ticket IDs from the PR description and include their details as review context. Requires `jiraBaseUrl`, `jiraEmail`, and `jiraApiToken`. |
| `jiraBaseUrl` | Conditional | - | Base URL of your JIRA Cloud instance (e.g. `https://myorg.atlassian.net`). Required if `includeWorkItems` is `true`. |
| `jiraEmail` | Conditional | - | Email address for JIRA Cloud API authentication. Required if `includeWorkItems` is `true`. |
| `jiraApiToken` | Conditional | - | JIRA Cloud API token (generate in your JIRA account settings). Required if `includeWorkItems` is `true`. |
...
When `includeWorkItems` is enabled and JIRA credentials are provided, the task extracts JIRA ticket IDs from the PR description (e.g. `PSDO-3897`) and fetches their details from JIRA Cloud to include as additional context for the code review.

### What gets included

- **Title** and **status** of each ticket
- **Description** (parsed from Atlassian Document Format to plain text)
- **Acceptance Criteria** (if populated on the ticket)
...
4. Store it as a secret pipeline variable (e.g. `JIRA_API_TOKEN`)

### Ticket ID extraction

The task scans the PR description for patterns matching JIRA ticket keys (e.g. `PROJ-123`, `PSDO-3897`). Any ticket IDs found are fetched automatically. If no IDs are found in the description, the task logs a message and continues the review normally.

### Behavior when JIRA is not configured

- If `includeWorkItems` is `false` (default): JIRA is completely skipped.
- If `includeWorkItems` is `true` but credentials are missing or incomplete: the task logs a warning and continues the review without work item context.
- If a specific ticket cannot be fetched (e.g. 404): it is silently skipped while others are still retrieved.
...
3. Add the relevant variables:
   - `GITHUB_PAT` (mark as secret) — GitHub PAT with Copilot access
...
1. **Install Copilot CLI**: The task ensures the GitHub Copilot CLI is installed on the build agent. Installed via `winget` (Windows) or the official install script (Linux).
2. **Fetch PR Context**: The task retrieves pull request metadata, file diffs, existing comments, and iteration details via the Azure DevOps REST API. If JIRA credentials are configured and `includeWorkItems` is enabled, JIRA ticket IDs are also extracted from the PR description and their details fetched from the JIRA Cloud API.
3. **Run Code Review**: Using the PR context and local Git commands, the Copilot CLI analyzes the changes using the configured or default prompt.
4. **Post Comments**: Review findings are posted as comments on the pull request via the Azure DevOps API.

For large PRs with many changed files, the task automatically splits the review into multiple chunks to fit within the model's context window.
...
- **Windows**: Requires `winget` to be available for Copilot CLI installation. On Linux, requires `curl` and `bash` (standard on most systems).
- **General Comments Only**: Posts general PR comments (file-level inline comments not yet supported)
- **Context Window**: Very large PRs are automatically chunked into multiple review passes
...
Contributions are welcome! Please feel free to submit issues or pull requests on [GitHub](https://github.com/biswajitsamal59/ado-copilot-code-review).
...
For issues and feature requests, please use the [GitHub Issues](https://github.com/biswajitsamal59/ado-copilot-code-review/issues) page.
...
- This project is not affiliated with or endorsed by GitHub or the Microsoft Corporation.
- All responses generated by GitHub Copilot remain subject to the probabilistic nature of the underlying LLM. As with all LLM-based interactions, there is a non-zero chance of unpredictable results. Use at your own discretion.
